...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Chart Editor", StringTools.replace(_song.song, '-', ' '), null, null, 'icon_editor');
		#end
	...
	function addSongUI():Void
	{
		...
		final uiSkins = UIData.getUIList();

		var uiSkinDropDown = new FlxUIDropDownMenu(stageDropDown.x, stageDropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(uiSkins, true),
			function(skin:String)
			{
				_song.uiSkin = uiSkins[Std.parseInt(skin)];
			});
		var skin = _song.uiSkin ?? '';
		uiSkinDropDown.selectedLabel = skin;
		...
		tab_group_song.add(new FlxText(uiSkinDropDown.x, uiSkinDropDown.y - 15, 0, 'UI Skin:'));
		tab_group_song.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 0, 'Stage:'));
		tab_group_song.add(player2DropDown);
		tab_group_song.add(gfVersionDropDown);
		tab_group_song.add(player1DropDown);
		tab_group_song.add(uiSkinDropDown);
	...
	function addDataUI()
	{
		...
		var check_disableNoteRGB:FlxUICheckBox = new FlxUICheckBox(10, 150, null, null, "Disable Note RGB", 100);
		...
		//
		noteSkinInputText = new FlxUIInputText(10, 190, 150, _song.arrowSkin != null ? _song.arrowSkin : '', 8);
		...
		composerInputText = new FlxUIInputText(10, reloadNotesButton.y + 40, 150, _song.composer != null ? _song.composer.join(',') : '', 8);
		...
		tab_group_data.add(composerInputText);
		...
		tab_group_data.add(new FlxText(composerInputText.x, composerInputText.y - 15, 0, 'Composer(s):'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			...
			else if (sender == composerInputText)
			{
				_song.composer = composerInputText.text.split(',');
			}
	...
	override function update(elapsed:Float)
	{
		...
		bpmTxt.text = Std.string(FlxMath.roundDecimal(Conductor.songPosition / 1000, 2))
			...
			+ curSec
			+ "\n\nBeat: "
			+ Std.string(FlxMath.roundDecimal(curDecBeat, 2))
			+ "\n\nStep: "
			+ curStep
			+ "\n\nBeat Snap: "
			+ quantization
	...
	public function doANoteThing(cs, d, style)
	{
		...
		{
			var point = FlxPoint.get(strumLineNotes.members[d].x + 1, strumLine.y + 1);
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.noteData == d % 4 && note.overlapsPoint(point))
				{
					// trace('tryin to delete note...');
					if (!delnote)
						deleteNote(note);
