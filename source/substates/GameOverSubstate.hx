...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	static var achievementName:String = '';
	...
	public static function resetVariables()
	{
		...
		{
			final mod = achievement.mod ?? '';
			if (mod == Mods.currentModDirectory && achievement.unlockCondition != null)
			{
				final split = achievement.unlockCondition.split(':');
				if (split[0] == 'song_death' && split[1] == Paths.formatToSongPath(PlayState.SONG.song))
				{
	...
	public function new(x:Float, y:Float, camX:Float, camY:Float)
	{
		...
		// this will only give one achievement at a time!!
		// there should only be one death achievement per song anyways
		if (achievementName.length > 0 && !Achievements.isAchievementUnlocked(achievementName))
		{
